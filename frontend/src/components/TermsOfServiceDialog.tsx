      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle className='mb-2 text-xl'>
            {t('termsofservice.enter_email')}
          </AlertDialogTitle>
        </AlertDialogHeader>

        <div className='mb-2'>
          <Input
            placeholder={t('termsofservice.email_placeholder')}
            value={email}
            onChange={(e) => {
        ...
        <div className='flex flex-col space-y-3 text-sm'>
          <p>
            {t('termsofservice.consent_text')}
            <a
              href='https://a.picoapps.xyz/camera-write'
              target='_blank'
              className='underline'
            >
              {t('termsofservice.terms_of_service')}
            </a>
            .
          </p>

          <p>
            {t('termsofservice.run_locally')}{' '}
            <a
              href='https://github.com/abi/screenshot-to-code'
              target='_blank'
              className='underline'
            >
              {t('termsofservice.download_github')}
            </a>
          </p>
        </div>
          ...
          <AlertDialogAction
            onClick={(e) => {
              if (!email.trim() || !email.trim().includes('@')) {
                e.preventDefault();
                toast.error(t('termsofservice.enter_email_error'));
              } else {
                onSubscribe();
              }
            }}
          >
            {t('termsofservice.agree_continue')}
          </AlertDialogAction>
        </AlertDialogFooter>
